...
const DutyTask = () => {
  ...
  const handleStationChange = async (e) => {
    ...
    if (station) {
      try {
        const response = await axios.get(`http://195.35.56.134:5000/api/usersForTask?area=${station}`);
    ...
    if (tasksToSubmit.length > 0) {
      try {
        ...
        for (const task of tasksToSubmit) {
          ...
          const response = await axios.post('http://195.35.56.134:5000/api/assignDuty', taskData);
